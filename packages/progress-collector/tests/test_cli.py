...
def document() -> CollectionDocument:
    period = TimeRange(datetime(2026, 7, 14, tzinfo=UTC), datetime(2026, 7, 15, tzinfo=UTC), "UTC")
    return CollectionDocument(
        period,
        {
            "github": SourceResult(
                ...
                (
                    Activity(
                        "1", "github.push", period.start, "Pushed a change", "https://example.test"
                    ),
                ),
            )
        },
...
def test_main_limits_sources_and_can_print_json(monkeypatch, capsys):
    ...
    monkeypatch.setattr(
        "progress_collector.cli._configs",
        lambda sources, config_file=None: (
            seen.update(sources=sources, config_file=config_file) or {}
        ),
    )
    ...
    assert seen["config_file"] == "/secure/auth.json"
...
def test_default_range_starts_at_utc_midnight_and_ends_now():
    ...
    assert _default_time_range(now) == TimeRange(datetime(2026, 7, 14, tzinfo=UTC), now, "UTC")
